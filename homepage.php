<?php
    ...
    if (isset($_POST['submit_leave'])) {
        ...
        // add on based leave_application table based on fields
                    ...
                    SELECT 
                        ...
                        COALESCE(
                            ...
                            if ($current_balance_result && $current_balance_result->num_rows > 0) {
                                ...
                                $new_vl = floatval($balance_row['current_vl']) + floatval($earned_vl);
                                $new_sl = floatval($balance_row['current_sl']) + floatval($earned_sl);
                            } else {
                                $new_vl = $balance_forwarded_vl + floatval($earned_vl);
                                $new_sl = $balance_forwarded_sl + floatval($earned_vl);
                            }
                            $sql = "INSERT INTO leave_applications 
